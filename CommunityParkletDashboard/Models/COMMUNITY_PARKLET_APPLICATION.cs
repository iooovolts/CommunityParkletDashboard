namespace CommunityParkletDashboard.Models
{
    public class COMMUNITY_PARKLET_APPLICATION
    {
        public int REF_NUMBER { get; set; }
        public string EMAIL { get; set; }
        public string NAME { get; set; }
        public string OTHER { get; set; }
        public string PARKLET_24HR_CONTACT { get; set; }
        public string PARKLET_DESCRIPTION { get; set; }
        public string PARKLET_MAINTENANCE { get; set; }
        public string PARKLET_TIME_FRAME { get; set; }
        public string PARKLET_TITLE { get; set; }
        public string PHONE { get; set; }
        public string TYPE_DISPLAY { get; set; }
        public string TYPE_EDUCATIONAL { get; set; }
        public string TYPE_FITNESS { get; set; }
        public string TYPE_GARDEN { get; set; }
        public string TYPE_OTHER { get; set; }
        public string TYPE_PLAY_AREA { get; set; }
        public string TYPE_REST_AREA { get; set; }
        public string C_POSTCODE_SEARCH__ADDRESS_NAME { get; set; }
        public string C_POSTCODE_SEARCH__CITY { get; set; }
        public string C_POSTCODE_SEARCH__COUNTY { get; set; }
        public string C_POSTCODE_SEARCH__LINE1 { get; set; }
        public string C_POSTCODE_SEARCH__LINE2 { get; set; }
        public string C_POSTCODE_SEARCH__LINE3 { get; set; }
        public string C_POSTCODE_SEARCH__POSTCODE { get; set; }
        public string C_POSTCODE_SEARCH__UPRN { get; set; }
        public string PARKLET__ADDRESS_NAME { get; set; }
        public string PARKLET__CITY { get; set; }
        public string PARKLET__COUNTY { get; set; }
        public string PARKLET__LINE1 { get; set; }
        public string PARKLET__LINE2 { get; set; }
        public string PARKLET__LINE3 { get; set; }
        public string PARKLET__POSTCODE { get; set; }
        public string PARKLET__UPRN { get; set; }
        public string NOTES { get; set; }
        public string STATUS { get; set; }
        public string FILE_NAME { get; set; }
    }
}